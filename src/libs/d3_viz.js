...
(function(root, factory) {
        ...
        bP.bars = function(mb){
            ...
            function calculateMainBars(part){
                ...
                ps.forEach(function(d,i){
                    mainBars[part].push({    // eslint-disable-next-line
                        x:_or=="horizontal"? (bars[i].s+bars[i].e)/2 : (part=="primary" ? bsize/2 : bP.width()-bsize/2)    // eslint-disable-next-line
                        ,y:_or=="vertical"? (bars[i].s+bars[i].e)/2 : (part=="primary" ? bsize/2 : bP.height()-bsize/2)    // eslint-disable-next-line
                        ,height:_or=="vertical"? (bars[i].e - bars[i].s)/2 : bsize/2    // eslint-disable-next-line
                        ,width: _or=="horizontal"? (bars[i].e - bars[i].s)/2 : bsize/2    // eslint-disable-next-line
                        ,part:part
                        ,key:d.key
                        ,value:d.value
                        ,percent:bars[i].p
            ...
            function calculateSubBars(part){
                ...
                ps.forEach(function(d){
                    var g= map.get(d.key);
                    var bars = bpmap(d.values, 0, 0    // eslint-disable-next-line
                    ...
                    d.values.forEach(function(t,i){
                        subBars[part].push({    // eslint-disable-next-line
                            x:_or=="vertical"? part=="primary" ? bsize/2 : bP.width()-bsize/2 : (bars[i].s+bars[i].e)/2    // eslint-disable-next-line
                            ,y:_or=="horizontal"? part=="primary" ? bsize/2 : bP.height()-bsize/2 : (bars[i].s+bars[i].e)/2    // eslint-disable-next-line
            ...
            function calculateEdges(){
                ...
                function edgeVert(x1,y1,x2,y2,x3,y3,x4,y4){    // eslint-disable-next-line
                ...
                function edgeHoriz(x1,y1,x2,y2,x3,y3,x4,y4){    // eslint-disable-next-line
                    if(eMode=="straight") return ["M",x1,",",y1,"L",x2,",",y2,"L",x3,",",y3,"L",x4,",",y4,"z"].join("")
    ...
    viz.gg = function(){    // eslint-disable-next-line
        ...
        gg.setNeedle =function(a){
            ...
            g.selectAll(".needleshadow").data([a])
                ...
                .attrTween("transform",function(d){  return iS(oldAngle,newAngle); })
                .ease(d3ease)
                .each("end",function(){this.angle=a;});
    ...
    viz.ch=function(){
        ...
        function ch(_){
            ...
            _.each(function() {
                ...
                grps.append("path")
                    ...
                    .attr("d", arc)
                    .each(function(d) { this._current = d; })    // eslint-disable-next-line
        ...
        function relayout(){
            keys =[];
            data.forEach(function(d){     // eslint-disable-next-line
            ...
            chords.forEach(function(d){     // eslint-disable-next-line
        ...
        function relayouts(fixedSource){    // eslint-disable-next-line
            ...
            newgroups.filter(function(g){ return g.type=="g"}).forEach(function(g, gi){
                ...
                var a0 = gia.map(function(d){ var k = keys[d];     // eslint-disable-next-line
            ...
            var m = d3.map(newchords.map(function(d){ return {startAngle:d.startAngle, endAngle:d.endAngle, indexsubindex:d.indexsubindex};})
                , function(d){ return d.indexsubindex;});    // eslint-disable-next-line
            var gmap = d3.map(newgroups.filter(function(d){ return d.type=="gs"}),function(d){ return d.source;});

            newchords.forEach(function(d){     // eslint-disable-next-line
                if(d.subindex == d.index){
        ...
        function viz_circularPartition(data, pad, min, fixedSource, startAngle){
            ...
            if(fixed){
                var found=false;
                for(;ind<data.length; ind++){     // eslint-disable-next-line
    ...
    viz.bar = function(){
        ...
        function bar(_){
            ...
            _.each(function() {
                ...
                var top_ =(dock=='t'), bot=(dock=='b'), left=(dock=='l'), rght=(dock=='r');    // eslint-disable-next-line
        ...
        bar.bars = function(){    // eslint-disable-next-line
            var data= bar.data();
            var h=bar.height(), w=bar.width();
            var dock = bar.dock();    // eslint-disable-next-line
            var top_ =(dock=='t'), bot=(dock=='b'), left=(dock=='l'), rght=(dock=='r');    // eslint-disable-next-line
    ...
    viz.lg = function(){
        function lg(_){
            ...
            _.each(function() {
                ...
                var rows = lg.rows(), data=lg.data();     // eslint-disable-next-line
    ...
    function viz_d1(d){ return d[1]}    // eslint-disable-next-line
    function viz_x(d){ return d.x}    // eslint-disable-next-line
    ...
    function viz_chord(rs, ss, se, re, es, ee) {
        ...
        return "M" + pss + arc(rs, pse, se - ss)     // eslint-disable-next-line
            + ((ss==es && se==ee) ? curve(pss,ss,se,re) : curve(pes,se,es,re) + arc(re, pee, ee - es) + curve(pss,ss,ee,rs)) + "Z";

        function arc(r, p, a) {  return "A" + r + "," + r + " 0 " + +(a > π) + ",1 " + p;   }
        function p(r,a){ return [r*Math.cos(a), r*Math.sin(a)]; }    // eslint-disable-next-line
