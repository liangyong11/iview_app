...
<script>
    ...
    export default {
        ...
        data: function () {
            return {
                dataSource: [
                    {className: '张三', chinese: 88, math: 92, physics: 90, chemistry: 88, history: 91, english: 95},
                    {className: '李四', chinese: 67, math: 78, physics: 80, chemistry: 72, history: 82, english: 74},
                    {className: '王五', chinese: 77, math: 83, physics: 68, chemistry: 69, history: 88, english: 65},
                    {className: '赵六', chinese: 72, math: 67, physics: 62, chemistry: 67, history: 72, english: 68},
                    {className: '刘七', chinese: 60, math: 58, physics: 76, chemistry: 63, history: 52, english: 70}
                ]
            };
        },
        methods: {
            loadChart: function () {
                ...
                let container = d3.select("#chart_radar").node();
                let containerWidth = container.offsetWidth - 100,
                    containerHeight = container.offsetHeight - 20,
                    chart = d3.select("#chart_radar").append("svg")
                        .attr("width", containerWidth)
                        .attr("height", containerHeight);

                const margin = {top: 100, right: 80, bottom: 20, left: 60};
                const width = containerWidth - margin.left - margin.right;
                const height = containerHeight - margin.top - margin.bottom;
                const outerRadius = Math.min(width, height) * 0.5;

                let g = chart.append("g").attr("transform", "translate(" + containerWidth / 2 + "," + (containerHeight + 40) / 2 + ")");

                const names = {chinese: '语文', math: '数学', physics: '物理', chemistry: '化学', history: '历史', english: '英语'};
